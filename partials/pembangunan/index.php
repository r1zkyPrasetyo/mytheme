<?php  if(!defined('BASEPATH')) exit('No direct script access allowed'); ?>
<!-- PAGE-HEADER -->
<div class="page-header">
    <div>
        <h1 class="page-title">Data Proses Pembangunan</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item" aria-current="page"><a href="<?= base_url(); ?>">Beranda</a></li>
        </ol>
    </div>
</div>
<!-- PAGE-HEADER END -->
<style>
   #border-pagination,#pagination{padding:0;text-align:center}#border-pagination li a,#pagination li a{text-decoration:none;color:#000;padding:5px 10px}#border-pagination,#pagination,.credit,.page-header{text-align:center}.container-berita{min-height:80vh}.img_url{height:150px!important;width:auto 100%!important;background-repeat:no-repeat;background-position:center;-webkit-background-size:cover;-moz-background-size:cover;-o-background-size:cover;background-size:cover}.page-header{font-size:1.5em;font-weight:400;border-bottom:1px solid #ddd;margin:30px 0}#pagination{margin:0}#border-pagination li,#pagination li{display:inline}#pagination li a{display:inline-block;border-radius:5px;-webkit-transition:background-color .3s;transition:background-color .3s}#border-pagination li a.active,#pagination li a.active{background-color:#4caf50;color:#fff}#pagination li a:hover:not(.active){background-color:#ddd}.b-pagination-outer{width:100%;margin:0 auto;text-align:center;overflow:hidden;display:flex}#border-pagination{margin:0 auto}#border-pagination li a{display:block;border:1px solid #ddd;float:left;-webkit-transition:background-color .4s;transition:background-color .4s}#border-pagination li a:hover:not(.active){background:#ddd}.empty-state{width:750px;margin:40px auto;background:#fff;box-shadow:1px 2px 10px #e1e3ec;border-radius:4px}.empty-state__content{padding:48px;display:flex;align-items:center;flex-direction:column}.empty-state__content .empty-state__icon{width:200px;height:200px;display:flex;align-items:center;border-radius:200px;justify-content:center;background-color:#f7fafc;box-shadow:0 2px 1px #e1e3ec}.empty-state__content .empty-state__icon img{width:170px}.empty-state__content .empty-state__message{color:#38a169;font-size:1.5rem;font-weight:500;margin-top:.85rem}.empty-state__content .empty-state__help{color:#a2a5b9;font-size:.875rem}.credit{color:#a2a5b9;font-size:.75rem}.credit a{color:#444}
</style>

<div class="row row-cards row-deck">
    <?php if ($pembangunan): ?>
    <?php foreach ($pembangunan as $data): ?>
    <div class="col-md-4">
        <div class="card">
            <div class="text-center p-2">
                <?php if (is_file(LOKASI_GALERI.$data->foto)): ?>
                <div class="img_url"
                    style="background-image:url('<?= base_url() . LOKASI_GALERI . $data->foto ?>') !important;">
                </div>
                <?php else: ?>
                <div class="img_url"
                    style="background-image:url('<?= base_url("$this->theme_folder/$this->theme/images/noimage.png") ?>') !important;">
                </div>
                <?php endif;?>
            </div>
            <div class="card-header border-top">
				<div class="card-title h5"><?= $data->judul ?></div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped" style="border: none;" cellspacing="0" cellpadding="0">
                        <tbody>
                            <tr>
                                <td>Lokasi</td>
                                <td>:</td>
                                <td><?= ($data->alamat == "=== Lokasi Tidak Ditemukan ===") ? 'Lokasi tidak diketahui' : $data->alamat; ?></td>
                            </tr>
                            <tr>
                                <td>Volume</td>
                                <td>:</td>
                                <td><?= $data->volume?> m</td>
                            </tr>
                            <tr>
                                <td>Anggaran</td>
                                <td>:</td>
                                <td>Rp. <?= number_format($data->anggaran,0) ?></td>
                            </tr>
                            <tr>
                                <td>Tahun</td>
                                <td>:</td>
                                <td><?= $data->tahun_anggaran ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <a href="<?= site_url("pembangunan/$data->slug"); ?>"
                    class="btn btn-primary btn-md">Lihat Data</a>
            </div>
        </div>
    </div><!-- COL-END -->
    <?php endforeach; ?>
    <?php else: ?>
        <div class="empty-state">
            <div class="empty-state__content">
                <div class="empty-state__icon">
                <img src="data:image/png;base64," alt="">
                </div>
                <div class="empty-state__message">Data Belum Ada</div>
                <div class="empty-state__help">
                Silakan kunjungi situs web kami dalam waktu dekat.
                </div>
            </div>
       </div>
    <?php endif; ?>
</div>